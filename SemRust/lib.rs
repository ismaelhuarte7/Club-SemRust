...
impl <'a> Fecha {
    fn new(dia: u64, mes: u8, año: u32) -> Result<Fecha,&'a str> {
        let f1 = Fecha { dia, mes, año };
        if f1.es_fecha_valida() { 
            return Ok(f1)
        } else {
            Err("La fecha ingresada no es válida.")
        }
    }
    ...
    ///Valida que la fecha cumpla las condiciones para ser creada
    fn es_fecha_valida(&self) -> bool { 
        match self.tipo_mes() {
            Ok(max_dia) => (self.dia <= max_dia as u64) && (self.mes <= 12),
            Err(_) => false, 
        }
    }
    ...
    #[test]
    fn test_new_fecha_valida() {
        let fecha_result = Fecha::new(9, 6, 2023);
        assert!(fecha_result.is_ok());
    }
    #[test]
    fn test_new_fecha_invalida() {
        let fecha_result = Fecha::new(31, 4, 2023);
        assert!(fecha_result.is_err());
    }
    #[test]
    fn test_tipo_mes() {
        let fecha = Fecha::new(1, 1, 2023).unwrap();
        assert_eq!(fecha.tipo_mes().unwrap(), 31);
        let fecha = Fecha::new(1, 2, 2023).unwrap();
        assert_eq!(fecha.tipo_mes().unwrap(), 28);
        let fecha = Fecha::new(1, 2, 2024).unwrap();
        assert_eq!(fecha.tipo_mes().unwrap(), 29);
        let fecha = Fecha::new(1, 4, 2023).unwrap();
        assert_eq!(fecha.tipo_mes().unwrap(), 30);
    }
        #[test]
        fn test_es_fecha_valida() {
            let fecha = Fecha::new(29, 2, 2023);
            assert_eq!(fecha.is_err(), true);
            let fecha = Fecha::new(29, 2, 2024);
            assert_eq!(fecha.is_ok(), true);
        }
        #[test]
        fn test_es_bisiesto() {
            let fecha = Fecha::new(1, 1, 2023).unwrap();
            assert_eq!(fecha.es_bisiesto(), false);
            let fecha = Fecha::new(1, 1, 2024).unwrap();
            assert_eq!(fecha.es_bisiesto(), true);
            let fecha = Fecha::new(1, 1, 2100).unwrap();
            assert_eq!(fecha.es_bisiesto(), false);
        ...
        #[test]
        fn test_sumar_dias() {
            let mut fecha = Fecha::new(30, 12, 2023).unwrap();
            fecha.sumar_dias(2);
            assert_eq!(fecha.dia, 1);
            assert_eq!(fecha.mes, 1);
            assert_eq!(fecha.año, 2024);
            let mut fecha = Fecha::new(31, 1, 2023).unwrap();
            fecha.sumar_dias(1);
            assert_eq!(fecha.dia, 1);
        ...
        #[test]
        fn test_es_mayor() {
            let fecha1 = Fecha::new(1, 1, 2023).unwrap();
            let fecha2 = Fecha::new(2, 1, 2023).unwrap();
            assert_eq!(fecha1.es_mayor(&fecha2), false);
            let fecha1 = Fecha::new(2, 1, 2023).unwrap();
            let fecha2 = Fecha::new(1, 1, 2023).unwrap();
            assert_eq!(fecha1.es_mayor(&fecha2), true);
            let fecha1 = Fecha::new(1, 2, 2023).unwrap();
            let fecha2 = Fecha::new(1, 1, 2023).unwrap();
            assert_eq!(fecha1.es_mayor(&fecha2), true);
            let fecha1 = Fecha::new(1, 1, 2023).unwrap();
            let fecha2 = Fecha::new(1, 2, 2023).unwrap();
            assert_eq!(fecha1.es_mayor(&fecha2), false);
            let fecha1 = Fecha::new(1, 1, 2023).unwrap();
            let fecha2 = Fecha::new(1, 1, 2023).unwrap();
            assert_eq!(fecha1.es_mayor(&fecha2), false);
